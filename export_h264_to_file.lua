do
	...
	-- Field主要作用是获取wireshark提供的各种报文属性
	...
	-- list 是nalu_type_list，index 是nalu的type。
    local function get_enum_name(list, index) 
        local value = list[index]
        return value and value or "Unknown"
    end
	
    -- menu action. When you click "Tools->Export H264 to file ['s plugins]" will run this function
    local function export_h264_to_file()
		...
		-- 创建文件，这一步是文件的名字
			...
			key = key:gsub(":", ".") -- 替换,将字符串中的所有":"全部替换成"." 
            local stream_info = stream_infos[key]
            if not stream_info then -- if not exists, create one
                stream_info = { }
                stream_info.filename = key.. ".h264"
                stream_info.file = io.open(stream_info.filename, "wb")  -- w代表如果这个文件不存在，那就创建它
			...
			--返回流的信息,包括src、dst以及他们的端口号
            return stream_info
        end
        
        -- write a NALU or part of NALU to file.
        local function real_write_to_file(stream_info, str_bytes, begin_with_nalu_hdr)
			-- first_run 如果是true,那么会先去发现sps+pps
            if first_run then
                stream_info.counter = stream_info.counter + 1 -- 统计NALU的总数量
                
                if begin_with_nalu_hdr then
                    -- save SPS or PPS
					-- str_bytes:byte(0,1)拿到str_bytes的第一个字节的ASCII值
					-- bit.band(a,b) 就是a和b按位与，拿到nalu的type
            ...
            -- 此时first_run 是false,证明已经发现sps+pps,该去保存了			
        ...
        local function sort_and_write(stream_info, frame) -- frame 是stream_info.frame_buffer
        ...
        local function process_stap_a(stream_info, h264, timestamp, seq)
            ...
            repeat
				...
				twappend("size = "..size)  -- 55296
				twappend("len = "..h264tvb:len())  --1183
                --write_to_file(stream_info, h264tvb(offset+2, size):string(), true, timestamp, i, true)
				if size+offset+2 < h264tvb:len() then 
    ...
    -- Find this feature in menu "Tools->"Export H264 to file [ZY's plugins]""
